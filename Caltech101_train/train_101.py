...
parser.add_argument('--gpu',required=True,type=str,default='4,5,6,7', help='gpu 설정')
...
dataset = 'caltech101' # cifar10 or cifar100
model = 'Resnet101' # resnet18, resnet50, resnet101
batch_size = 16  # Input batch size for training (default: 128)
...
transform = transforms.Compose([
    ...
    transforms.transforms.RandomCrop(112)
])

root1 = '###101_ObjectCategories 데이터 위치###'
root2 = '###101_ObjectCategories test 데이터 위치###'
...
def train(train_loader, epoch, model, optimizer, criterion):
    ...
    for i, (input, target, root) in enumerate(train_loader):
        ...
        for j in range(0,len(root)):
            ...
            weight=[8,11,10,7,5,9,6,4,3,2,1]
            rand=random.choices(range(0,11),weights=weight)[0]
            transparency=transparency_list[rand]
            root_j=root[j]
            if transparency==0:
                root_get=root_j[0:20]+'101_ObjectCategories/'+root_j.split('/')[-2]+'/'+root_j.split('/')[-1]
                new_img=Image.open(root_get)
            elif transparency==100:
                root_get=root_j[0:20]+'color_result_101/train_new/'+root_j.split('/')[-2]+'/'+root_j.split('/')[-1]
                new_img=Image.open(root_get)
            else:
                root_get=root_j[0:20]+'101_color_white_result_'+str(transparency)+'/'+root_j.split('/')[-2]+'/'+root_j.split('/')[-1][:-4]+'_'+str(transparency)+'.png'
                new_img=Image.open(root_get)
...
for epoch in range(epochs):
    ...
    if best_acc < test_acc:
        best_acc = test_acc
        torch.save(model.state_dict(), '101_model_best_color_white.pt')

    torch.save(model.state_dict(),'101_model_latest_color_white.pt')
    print(f"Best Top-1 Accuracy: {best_acc}")


import csv
with open('### csv 저장 경로/101_train_color_white.csv###', 'w',newline='') as f: 
    write = csv.writer(f) 
